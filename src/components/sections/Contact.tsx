...
const Contact = () => {
  ...
  const getButtonContent = () => {
    switch (status) {
      case "loading":
        return (
          <>
            <Loader size={17} className="animate-spin" /> Sending your message…
          </>
        );
      case "success":
        return (
          <>
            <CheckCircle size={17} /> Message Sent! I'll reply soon{" "}
          </>
        );
      case "error":
        return (
          <>
            <AlertCircle size={17} /> Something went wrong — Try again
          </>
        );
      default:
  ...
  const getButtonClass = () => {
    const base =
      "w-full flex items-center justify-center gap-2 h-10 px-4 rounded-md font-medium text-sm transition-all duration-300";
    switch (status) {
      case "success":
      ...
      case "loading":
        return `${base} opacity-70 cursor-not-allowed bg-white text-black`;
      default:
        return `${base} bg-white hover:bg-white/90 text-black shadow-sm`;
    }
  };
  ...
  return (
    <section
      id="contact"
      className="section-padding relative overflow-hidden dot-matrix"
    >
      {/* Subtle background glow */}
      <div className="absolute inset-0 -z-10 pointer-events-none">
        <div
          className="absolute bottom-0 right-0 w-[500px] h-[500px] rounded-full blur-[130px] opacity-[0.08]"
          style={{
            background:
        ...
        />
        <div
          className="absolute top-0 left-0 w-[400px] h-[400px] rounded-full blur-[130px] opacity-[0.06]"
          style={{
            background:
          ...
          >
            ...
            <div className="premium-card p-6">
              ...
              <p className="text-sm text-white/50 leading-normal mb-5">
                ...
                and <strong className="text-foreground">full-time roles</strong>
                . I bring full-stack expertise and ship fast without sacrificing
                quality.
              </p>
              {/* Contact details */}
